...
class ApplicationSpec extends WordSpec
  ...
  "Application Controller" should {
    ...
    "render the index page" in {
      val index = route(FakeRequest(GET, versionCtx + "/toggle-service")).get

      status(index) shouldBe OK
      contentType(index).get == "application/json" shouldBe true
      (contentAsJson(index) \ "response" \ "results").as[String] == "toggle-service is up and running!" shouldBe true
    }
    ...
    "change the log Level" in {
      val changeLog = route(FakeRequest(GET, versionCtx + "/toggle-service/logLevel/WARN")).get
      ...
      Await.result(route(FakeRequest(GET, versionCtx + "/toggle-service/logLevel/DEBUG")).get, 10 seconds)
      Logger.isDebugEnabled shouldBe true
    }

    "ignore incorrect log Level" in {
      route(FakeRequest(GET, versionCtx + "/toggle-service/logLevel/WARN2")) shouldBe None
    }
  }
}
