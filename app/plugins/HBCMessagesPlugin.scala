...
import play.api.i18n.{DefaultMessagesPlugin, Messages}
...
class HBCMessagesPlugin(app: Application) extends DefaultMessagesPlugin(app) {
  ...
  private lazy val bannerMessagesPath = joinPaths(Some(GLOBAL_MESSAGES_PATH), configuration.getString(BANNER_CONF).get)
