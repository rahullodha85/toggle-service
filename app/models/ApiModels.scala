...
class ApiRequestModelFactory(configuration: Configuration) {
  // TODO look into mocking this config in tests

  private val versionURI = configuration.getString("application.context").get
...
object ApiRequestModelFactory extends ApiRequestModelFactory(ConfigurationProvider.getConfiguration)
...
object ApiModel {
  ...
  def withHeader(body: String)(implicit req: RequestHeader): ApiModel = {
    val reqModel = ApiRequestModelFactory.fromReq(req)
    fromBody(body)
